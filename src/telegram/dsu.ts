...
import { daysAwayFromManila, formatEventCountdown, formatManilaLongDate } from '../lib/date'
import { escapeHtml, isCompletedStatus, isLikelyRiskStatus, isMissingRelationError, normalizeStatus } from '../lib/validate'
import type { ConfigMap, DashboardConfigRow, DashboardEvent, TaskRow, TelegramListItem } from '../types'
...
export function eventToTelegramListItem(event: DashboardEvent): TelegramListItem {
  return { chapter: event.chapter, title: event.event_name, status: event.status, dueDate: event.event_date }
}
...
export async function buildDsuMessage(supabase: ReturnType<typeof supabaseClient>): Promise<string> {
  const [configResult, eventResult, riskResult, taskResult, updateResult] = await Promise.all([
    ...
    supabase.from('dashboard_events')
      .select('slug, chapter, event_name, event_date, event_kind, status, pax_target, display_order')
    ...
    supabase.from('updates')
      ...
      .eq('is_risk', true).order('created_at', { ascending: false }).limit(6),
    supabase.from('tasks')
  ...
  const hardErrors = [configResult, eventResult, riskResult, taskResult, updateResult]
    .map((r) => r.error).filter((e) => e && !isMissingRelationError(e))

  if (hardErrors.length) return `Failed to load DSU data: ${hardErrors[0]?.message || 'unknown error'}`
  ...
  const completedCamps = scheduleEvents.filter((e) => isCompletedStatus(String(e.status || ''))).length
  ...
  const projectCompletion = configNumber(config, ['projects_completed', 'project_completion_count', 'confirmed_deployments'], completedProjects)
  ...
  const technicalDefault  = updates[0] ? `${updates[0].title} (${updates[0].status})` : 'No technical status available'
  ...
  if (!scheduleEvents.length) {
    ...
    for (const item of scheduleEvents.slice(0, 8)) {
      ...
      lines.push('', `${escapeHtml(item.chapter)} (Venue: ${escapeHtml(venue)})`, formatEventCountdown(item.event_date, item.status), `Lead: ${escapeHtml(lead)}`)
    }
  }

  lines.push('', '⚠️ HIGH RISKS & BLOCKERS')

  const riskRows = (riskResult.data || []) as Array<{ chapter: string; owner: string; title: string; status: string; notes: string | null; event_date: string | null; created_at: string }>
  ...
  if (riskRows.length) {
    ...
    const fallbackRisks = scheduleEvents
      .filter((e) => isLikelyRiskStatus(String(e.status || '')) && !isCompletedStatus(String(e.status || '')))
      .slice(0, 5)
  ...
  if (!openTasks.length) {
    ...
    for (const task of openTasks.slice(0, 24)) {
      ...
      byChapter.set(ch, [...(byChapter.get(ch) || []), task])
    }
    for (const [chapter, tasks] of byChapter.entries()) {
