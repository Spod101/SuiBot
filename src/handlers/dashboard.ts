...
import { daysAwayFromManila, formatManilaLongDate } from '../lib/date'
import { isCompletedStatus, isMissingRelationError, parseNumberOrDefault } from '../lib/validate'
import type { DashboardConfigRow, DashboardEvent } from '../types'
...
export async function handleDashboard(): Promise<Response> {
  ...
  if (configError && !isMissingRelationError(configError)) return jsonResponse({ error: configError.message }, 500)
  if (eventError  && !isMissingRelationError(eventError))  return jsonResponse({ error: eventError.message  }, 500)
  ...
  const codeCampsCompleted = codeCampEvents.filter((e) => isCompletedStatus(String(e.status || ''))).length
  ...
  return jsonResponse({
    ...
    manilaDateLabel,
    q2DaysRemaining,
    summary: { codeCampsTotal, codeCampsCompleted, formSubmissions, trainedMentors, confirmedDeployments, completionRatePct, computerLabs },
    events: events.map((e) => ({
      slug: e.slug,
      chapter: e.chapter,
...
export async function handleMetrics(): Promise<Response> {
  const supabase = supabaseClient()
  const { data, error } = await supabase.from('updates').select('status, is_risk, pax_actual')

  if (error) return jsonResponse({ error: error.message }, 500)

  const rows = data || []
  let completed = 0, inProgress = 0, openRisks = 0, totalPaxActual = 0

  for (const row of rows) {
    const status = String(row.status || '').trim().toLowerCase()
    if (isCompletedStatus(status)) completed++
    if (status === 'in progress' || status === 'in_progress' || status === 'ongoing') inProgress++
    if (row.is_risk) openRisks++
    totalPaxActual += Number(row.pax_actual || 0)
  }

  return jsonResponse({ totalUpdates: rows.length, completed, inProgress, openRisks, totalPaxActual })
}
